...
%  M (recommended value 20)                   the spatial size of each sub cube (line*column=M*M)
...
%   for i=1:stepszie:m-M+1                             % �������
%     for j=1:stepszie:n-M+1                         % �������
for   rownumber =1:row
...
%         [clear_patch_block S iter] = inexact_alm_rpca( patch_block, lambda, tol, maxIter);  %����RPCA�������


%            Varsig = cov(patch_block(:));
%            SVarnoi = sqrt(Varsig/30); 
%            mu = (M + sqrt(p)) * SVarnoi;
%           [clear_patch_block E_hat iter] = HWapg( patch_block, lambda, mu, tol);   % ����APG���
              for m2=1:1:p
